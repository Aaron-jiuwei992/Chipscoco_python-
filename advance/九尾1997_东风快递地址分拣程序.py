# - * - coding: utf - 8 -*-
# author: jiuwei1997
# description: 东风快递地址分拣程序
# date: 2021-4-17


def address_sorting_system(data):
    provinces = [
    ...
    private_info = dict(data)
    new_private_info = {}
    for k, v in private_info.items():
        for province in provinces:
            # 对省份进行切片保证不出现错误匹配
            if province in v[:7] or province[:3] in v[:7] or province[:2] in v[:7]:
                # 当新的字典中不存在某键时，构建一个以省份为键，以相应信息为值的键值对
                if not new_private_info.get(province):
                    new_private_info[province] = [[k, v]]
                    break
                # 当新的字典存在某个键时，直接添加键值对
                else:
                    new_private_info[province].append([k, v])
                    break

    print(new_private_info)
